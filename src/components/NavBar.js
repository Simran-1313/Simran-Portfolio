...
import  {TwitterIcon, Instagram, GithubIcon, LinkedInIcon, PinterestIcon } from './icons';
...
const NavBar = () => {
  ...
  return (
    ...
    >
      ...
      <span className={`bg-dark block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm m-0.5 ${isOpen?'opacity-0':'opacity-100'}`} ></span>
      <span className={`bg-dark block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm  ${isOpen?'-rotate-45 -translate-y-1':'translate-y-0.5'} `} ></span>
      </button>
    ...
    <nav className='flex items-center justify-center flex-wrap'>
      ...
      <motion.a href="https://www.instagram.com/singhsimranpreet916" target= {"_blank"}
      whileHover={{y:-2}}
      whileTap={{scale:0.9}}
      className='w-6 ml-3'
      >
